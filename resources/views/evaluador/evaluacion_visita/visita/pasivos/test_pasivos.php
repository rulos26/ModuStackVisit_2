<?php
// Archivo de prueba para el módulo de Pasivos
// Este archivo verifica todas las funcionalidades del módulo

echo "<h1>🧪 PRUEBAS DEL MÓDULO PASIVOS</h1>";
echo "<style>
...
try {
    ...
    // Verificar que la tabla pasivos existe
    $stmt = $db->query("SHOW TABLES LIKE 'pasivos'");
    if ($stmt->rowCount() > 0) {
        echo "<div class='test-result success'>✅ Tabla 'pasivos' existe</div>";
        
        // Verificar estructura de la tabla
        $stmt = $db->query("DESCRIBE pasivos");
        $columns = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo "<div class='test-result info'>📋 Estructura de la tabla pasivos:</div>";
        echo "<pre>";
        ...
        echo "<div class='test-result error'>❌ Tabla 'pasivos' no existe</div>";
    }
...
try {
    ...
    echo "<div class='test-result success'>✅ Controlador PasivosController cargado correctamente</div>";
...
try {
    ...
    if (!empty($datos_existentes)) {
        ...
        echo "<div class='test-result info'>📊 Total de pasivos: " . count($datos_existentes) . "</div>";
        
        echo "<div class='test-result info'>📋 Primer pasivo:</div>";
        echo "<pre>";
...
try {
    ...
    // Datos válidos - Múltiples pasivos
    $datos_validos_multiple = [
        'item' => ['Tarjeta de crédito', 'Préstamo hipotecario'],
        'id_entidad' => ['Banco de Bogotá', 'Bancolombia'],
        'id_tipo_inversion' => ['Consumo', 'Hipotecario'],
        'id_ciudad' => ['1', '2'],
        'deuda' => ['5000000', '50000000'],
        'cuota_mes' => ['150000', '800000']
    ];
    
    $errores_validos_multiple = $controller->validarDatos($datos_validos_multiple);
    if (empty($errores_validos_multiple)) {
        echo "<div class='test-result success'>✅ Validación de múltiples pasivos exitosa</div>";
    } else {
        echo "<div class='test-result error'>❌ Error en validación de múltiples pasivos:</div>";
    ...
    $datos_invalidos = [
        'item' => ['AB'], // Muy corto
        'id_entidad' => ['XY'], // Muy corto
        'id_tipo_inversion' => ['ZZ'], // Muy corto
        'id_ciudad' => ['0'], // ID inválido
        'deuda' => ['-1000'], // Negativo
        'cuota_mes' => ['invalid'] // No numérico
    ];
    ...
    $datos_longitud_diferente = [
        'item' => ['Pasivo 1', 'Pasivo 2'],
        'id_entidad' => ['Entidad 1'], // Solo uno
        'id_tipo_inversion' => ['Tipo 1', 'Tipo 2'],
        'id_ciudad' => ['1', '2'],
        'deuda' => ['1000000', '2000000'],
        'cuota_mes' => ['50000', '100000']
    ];
...
try {
    $datos_sucios = [
        'item' => ['  <script>alert("xss")</script>Tarjeta de crédito  '],
        'id_entidad' => ['  <script>alert("xss")</script>Banco de Bogotá  '],
        'id_tipo_inversion' => ['  <script>alert("xss")</script>Consumo  '],
        'id_ciudad' => ['1'],
        'deuda' => ['  <script>alert("xss")</script>5000000  '],
        'cuota_mes' => ['  <script>alert("xss")</script>150000  ']
    ];
...
$campos_requeridos = [
    'item', 'id_entidad', 'id_tipo_inversion', 'id_ciudad', 'deuda', 'cuota_mes'
];
...
echo "<div class='test-result info'>🎯 Redirección configurada a: ../aportante/aportante.php</div>";

// Verificar funcionalidades dinámicas
echo "<div class='test-result info'>📋 Funcionalidades dinámicas:</div>";
echo "<div class='test-result success'>✅ Agregar múltiples pasivos</div>";
echo "<div class='test-result success'>✅ Eliminar pasivos individuales</div>";
echo "<div class='test-result success'>✅ Validación de campos por pasivo</div>";
echo "<div class='test-result success'>✅ Carga de datos existentes</div>";
echo "<div class='test-result success'>✅ Formateo automático de campos monetarios</div>";

// Verificar casos especiales
echo "<div class='test-result info'>📋 Casos especiales manejados:</div>";
echo "<div class='test-result success'>✅ Sin pasivos: Guarda con valores 'N/A'</div>";
echo "<div class='test-result success'>✅ Con pasivos: Guarda datos detallados</div>";
...
echo "<div class='test-result success'>✅ Módulo de Pasivos refactorizado correctamente</div>";
...
echo "<div class='test-result success'>✅ Funcionalidad de múltiples pasivos implementada</div>";
echo "<div class='test-result success'>✅ Interfaz dinámica con JavaScript</div>";
echo "<div class='test-result success'>✅ Manejo de casos especiales (con/sin pasivos)</div>";
echo "<div class='test-result info'>🎯 Próximo módulo: Aportante (aportante.php)</div>";
echo "</div>";

echo "<div class='test-section warning'>";
echo "<h3>⚠️ PRÓXIMOS PASOS</h3>";
echo "<div class='test-result warning'>1. Verificar que el módulo de Aportante existe en ../aportante/aportante.php</div>";
...
echo "<div class='test-result warning'>4. Probar la funcionalidad de agregar/eliminar pasivos</div>";
echo "<div class='test-result warning'>5. Verificar que se cargan correctamente los datos existentes</div>";
echo "<div class='test-result warning'>6. Probar ambos casos: con pasivos y sin pasivos</div>";
echo "</div>";
?> 