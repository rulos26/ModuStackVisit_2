...
// Test 1: Verificar que la versión simple funciona
echo "<h3>1. Test de la Versión Simple</h3>";
echo "<button onclick='probarVersionSimple()' class='btn btn-success'>Probar Versión Simple</button>";
echo "<div id='resultadoSimple' class='mt-3'></div>";

// Test 2: Verificar que la versión original funciona
echo "<br><h3>2. Test de la Versión Original</h3>";
echo "<button onclick='probarVersionOriginal()' class='btn btn-primary'>Probar Versión Original</button>";
...
<script>
function probarVersionSimple() {
    const resultadoDiv = document.getElementById('resultadoSimple');
    resultadoDiv.innerHTML = '<div class="alert alert-info">Probando versión simple...</div>';
    ...
    .then(text => {
        try {
            ...
            if (data.error) {
                ...
                resultadoDiv.innerHTML = `<div class="alert alert-success">✅ Versión Simple funciona. Usuarios: ${data.length}</div>`;
            }
        } catch (parseError) {
